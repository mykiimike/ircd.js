...
User.prototype = {
  ...
  hostLookup: function() {
    ...
    dns.reverse(this.remoteAddress, function(err, addresses) {
      if(user.isHide === true)
        user.oldHostname = addresses && addresses.length > 0 ? addresses[0] : user.remoteAddress;
      else
  ...
  addMode: {
    ...
    x: function() {
      if (!this.modes.match(/x/)) {
        this.isHide = true;
        this.oldHostname = this.hostname;

        /* computing hash mask */
        var ct = crypto.createHash('sha256');
        ct.update(this.server.config.sharedKey);
        ct.update(this.hostname);
        this.hostname = ct.digest('hex');
  ...
  removeMode: {
    ...
    x: function(user, arg) {
      if (this.modes.match(/x/)) {
        this.isHide = false;
        this.hostname = this.oldHostname;
        user._modes.splice(user._modes.indexOf('x'), 1);
        this.send(this.mask, 'MODE', this.nick, '-x', this.nick);
      }
    }
