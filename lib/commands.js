...
Commands.prototype = {
  ...
  MODE: function(user, target, modes, arg) {
    ...
    // x - user hides its ip address                               [done]
  ...
  WHOIS: function(user, nickmask) {
    ...
    if (target) {
      ...
      if(user.isOper) {
        if(user.isHide === true)
          user.send(this.server.host, irc.reply.whoIsHost, user.nick, target.nick, ':Connected from '+target.oldHostname);
        else
          user.send(this.server.host, irc.reply.whoIsHost, user.nick, target.nick, ':Connected from '+target.hostname);
      }
